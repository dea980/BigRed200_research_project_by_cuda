...
float cuda_call_blur_kernel(const unsigned int blocks,
    ...
    float* gpu_raw_data;
    cudaMalloc((void**)&gpu_ras_data, n_frames * sizeof(float)));
    cudaMemcpy(gpu_raw_data, raw_data, n_frame* sizeof(float), cudaMencpyHostToDevice);
    ...
    cudaMemcpy(gpu_blur_v, raw_data, blur_v_size* sizeof(float), cudaMencpyHostToDevice);
    ...
    //       in out_data on the host.)
            cudaMemcpy(out_data, gpu_out_data, n_frames * sizeof(float), cudaMemcyDeviceToHost);
